...
public class ZipRunner {
    public static void main(String[] args) {
        ...
        System.out.println("Запуск архиватора...");
        ...
        Arrays.stream(args).findFirst().ifPresentOrElse(file -> {
            final String zip = archiveService.zip(file).orElseThrow(() -> new IllegalStateException("Не удалось выполнить архивацию"));
            System.out.printf("Архиватор успешно выполнил свою работу. Результат %s", zip);
        }, () -> {
            System.out.println("Укажите архивируемый файл в качестве параметра запуска программы");
            System.exit(0);
        });
    }
}
